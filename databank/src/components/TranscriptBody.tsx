...
export default function TranscriptBody({ text }: { text: string }) {
  ...
  return (
    <div className="prose-transcript space-y-4 text-[15px] leading-relaxed text-ink-700">
      {blocks.map((b, i) => {
        switch (b.kind) {
          case 'heading':
            return (
          ...
          case 'quote':
            return (
              <figure key={i} className="rounded-2xl bg-cream-50 border border-cream-200 px-5 py-4">
                ...
                <blockquote className="mt-1.5 text-ink-900">{b.text}</blockquote>
              </figure>
            )
          case 'question':
            return (
              <div key={i} className="rounded-2xl bg-white border-l-4 border-moss-500 border-y border-r border-cream-200 px-5 py-4">
                ...
                <p className="mt-1.5 font-bold text-ink-900">{b.text}</p>
              </div>
            )
          case 'insight':
            return (
